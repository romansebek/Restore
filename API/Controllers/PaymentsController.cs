...
public class PaymentsController(PaymentsService paymentsService, StoreContext context, IConfiguration config, ILogger<PaymentsController> logger) : BaseApiController
{
    ...
    public async Task<IActionResult> StripeWebhook()
    {
        ...
        {
            logger.LogError(ex, "Stripewebhook error");
            return StatusCode(StatusCodes.Status500InternalServerError, "Webhook error");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Unecpected error has occured");
            return StatusCode(StatusCodes.Status500InternalServerError, "Unecpected error");
        }
    }
